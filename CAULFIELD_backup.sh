...
read -p "Enter backup target:" fn

if [ ! -d "$fn" ]; then
    echo "Error: Directory $fn doesn't exist here."
    exit 1
fi

for fil in "$fn"/*; do
    if [ -f "$fil" ]; then
        cp "$fil" "$HOME/BACKUP_FILES"
    fi
done
...
print_info () {
    echo "Created BACKUP_FILES in $HOME directory"
    for file in "$HOME/BACKUP_FILES"/*; do
        echo "$(basename "$file") copied to $HOME/BACKUP_FILES"
    done
    ls -alh "$HOME/BACKUP_FILES"
    du -sh "$HOME/BACKUP_FILES" | awk '{print $1, $2, $3}'
}
print_info
